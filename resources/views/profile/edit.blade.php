...
<body class="bg-gray-100 flex items-center justify-center min-h-screen">
    <div class="bg-white rounded-lg shadow-lg p-6 max-w-sm w-full relative">
        <h1 class="text-xl font-bold mb-4 text-center">Edit Profile</h1>
        <a href="{{ route('dashboard') }}" class="absolute top-4 right-4 text-gray-500 hover:text-gray-700">
        ...
        <form action="{{ route('profile.update') }}" method="POST" class="space-y-4">
            ...
            <button type="submit" class="w-full bg-blue-500 hover:bg-blue-600 text-white rounded-lg p-2">Update Profile</button>
        </form>

        <form action="{{ route('profile.destroy') }}" method="POST" class="mt-4">
            @csrf
            @method('DELETE')
            <button type="submit" class="w-full bg-red-500 hover:bg-red-600 text-white rounded-lg p-2">Delete Account</button>
        </form>
    </div>
</body>
</html>
